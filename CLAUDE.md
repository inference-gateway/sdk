...
The project uses [Task](https://taskfile.dev) (see `Taskfile.yml`). Toolchain is pinned via Flox in `.flox/env/manifest.toml` (Go 1.26.7, golangci-lint ^2.12.2) — `flox activate` from the repo root puts the right versions on `PATH`. CI runs `golangci-lint run --timeout 5m`, `go build -v ./...`, and `go test -v ./...`.
...
- **`generated_types.go`** — **generated, do not hand-edit.** Marked `linguist-generated=true` in `.gitattributes`. Contains every request/response struct (`CreateChatCompletionRequest`, `CreateChatCompletionResponse`, `Message`, `MessageContent` union, `ContentPart`, `ChatCompletionTool`, `FunctionObject`, `Model`, `MCPJSONRPCRequest`, `Error`, `SSEvent`, …) and the enums (`Provider`, `MessageRole`, `FinishReason`, `ImageURLDetail`, `SSEventEvent`, …) with `Valid()` methods. To change anything in here, edit the upstream OpenAPI in `inference-gateway/schemas`, then `task oas-download && task generate`.
